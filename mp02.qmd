...
p <- ggplot(growth_stats, aes(
  x = composite_growth,
  y = pop_growth,
  text = paste0(
    NAME,
    "<br>Δ Pop: ", scales::comma(pop_growth),
    "<br>Composite: ", round(composite_growth, 3)
  )
)) +
...
p <- ggplot(summary_burden, aes(
  ...
  text = paste0(
    "CBSA: ", NAME,
    "<br>Δ Rent: ", round(delta_rent, 0),
    "<br>Composite Growth: ", round(composite_growth, 3),
    "<br>Pop Growth: ", scales::comma(pop_growth)
  )
)) +
  geom_point(alpha = 0.7) +
  labs(
...
# Convert to interactive version with hover tooltips
ggplotly(p, tooltip = "text")
